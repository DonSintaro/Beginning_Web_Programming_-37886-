...
//indicates strict use of syntax
...
var thisDate = new Date();                            //creates var holding date object
var dateString = thisDate.toLocaleDateString();       //creates var holding local convention date
var dateHTML = ("<h2>" + dateString + "</h2>");       //creates var holding html ready date using previous var
var thisDay = thisDate.getDay();                      //creates var holding the day number value that can be used for getEvent()
var eventHTML = getEvent(thisDay);                    //creates var holding the product of getEvent() using thisDay var in parameter

document.getElementById("unionToday").insertAdjacentHTML("beforeEnd",(dateHTML + eventHTML));  //inserts string data into the html file


function getEvent(day) {               //function that returns html text based on the "day" var in the parameter
   var eventHTML;
   switch (day) {                      //switch statement to return based on number value provided
   ...
   return eventHTML   //returns product from switch statement and end function.
}
